...
<body>
    ...
    let result =
        u8::from_selector_and_attr("a", "href", html.select(&selector).next().unwrap()).unwrap();
...
<body>
    ...
    let selector = Selector::parse("#test").unwrap();
    let result =
        u8::from_selector_and_inner_text("a", html.select(&selector).next().unwrap()).unwrap();
    assert_eq!(1u8, result);
}
...
<body>
    ...
    let selector = Selector::parse("#test").unwrap();
    let result =
        String::from_selector_and_html("a", html.select(&selector).next().unwrap()).unwrap();
